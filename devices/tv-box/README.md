...
Customer onboarding, Wi-Fi password recovery, operator acceptance, and troubleshooting procedures are in [PILOT_RUNBOOK.md](PILOT_RUNBOOK.md).
...
The copied appliance payload directories must also be free of ignored local files. Build checks reject those files even when Git reports a clean working tree; only regular Python bytecode in the runtime package is allowed because the image layer removes it. Reference documents and build artifacts outside the copied payload directories are not image inputs.
...
- `hexclave-tv-box-relay-identity.service` independently creates a unique relay-only key after first boot. `hexclave-tv-box-relay.service` is activated only by explicit per-device enrollment; its short validator replaces itself with OpenSSH, and systemd owns automatic retries. No Python relay supervisor stays resident; neither service is a dependency of Wi-Fi, the renderer, or local SSH.
...
Office support can use the optional [reverse-SSH transport](PILOT_RUNBOOK.md#optional-support-relay). A configurable, trusted OpenSSH endpoint, per-device enrollment and operator authentication are required; there is no hosting-provider assumption or need for another backend. An isolated relay service may run on an approved existing host. The base image is unenrolled and makes no support connection. One relay-loopback listener reaches the appliance's loopback SSH service, preserving its certificate/forced-command boundary. Local support remains independently available from private/link-local sources on Wi-Fi, not globally routed addresses. This address rule is not a claim of same-subnet enforcement; a Mac can continue using the Pi's private LAN IPv4 address with no relay or jump host.
