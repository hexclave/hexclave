...
<ClickableTableOfContents
  ...
  & {
    ...
    selectedTeam: ServerTeam | null; //$stack-link-to:#serveruserselectedteam
...
>
  <MethodLayout>
    <MethodContent>
      The currently selected team as a `ServerTeam`, or `null` if no team is selected. Server-only functionality,
      such as `addUser()` or the full member details returned by `listUsers()`, is available on it directly,
      without fetching the team again with `hexclaveServerApp.getTeam()`.
    </MethodContent>
    <MethodAside title="Type Definition">
      ```typescript declare const selectedTeam: ServerTeam | null; ```
    </MethodAside>
  </MethodLayout>
</ServerUserSection>
