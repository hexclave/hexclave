-- TV legacy purchase index recovery tolerates invalid remnants from interrupted concurrent builds.
...
BEGIN
  ...
  WHERE index_namespace.nspname = current_schema()
    AND index_relation.relname = 'temp_OneTimePurchase_legacyPurchasePage_createdAt_idx_invalid';
  ...
  IF FOUND AND (
    ...
    OR existing.index_definition NOT LIKE '%("tenancyId", "createdAt") WHERE%'
    OR regexp_replace(regexp_replace(existing.predicate_definition, '("[^"]+"|[A-Za-z_][A-Za-z0-9_$]*)\."PurchaseCreationSource"', '"PurchaseCreationSource"', 'g'), '[()\s]', '', 'g') IS DISTINCT FROM regexp_replace('"creationSource" = ''PURCHASE_PAGE''::"PurchaseCreationSource" AND "paidAt" IS NULL', '[()\s]', '', 'g')
    ...
    RAISE EXCEPTION 'TV legacy purchase invalid-index remnant % exists but has an unexpected definition; refusing to drop it', 'temp_OneTimePurchase_legacyPurchasePage_createdAt_idx_invalid';
  END IF;
END
$$;
...
DROP INDEX CONCURRENTLY IF EXISTS /* SCHEMA_NAME_SENTINEL */."temp_OneTimePurchase_legacyPurchasePage_createdAt_idx_invalid";
...
BEGIN
  ...
  LEFT JOIN pg_am access_method ON access_method.oid = index_relation.relam
  WHERE index_namespace.nspname = current_schema()
    AND index_relation.relname = 'temp_OneTimePurchase_legacyPurchasePage_createdAt_idx';
  ...
  IF FOUND AND (
    ...
    RAISE EXCEPTION 'TV legacy purchase index % already exists but is invalid or has an unexpected definition; refusing to drop it', 'temp_OneTimePurchase_legacyPurchasePage_createdAt_idx';
  ELSIF FOUND AND (
    existing.indisvalid IS DISTINCT FROM TRUE
    OR existing.indisready IS DISTINCT FROM TRUE
  ) THEN
    EXECUTE format('ALTER INDEX %I RENAME TO %I', 'temp_OneTimePurchase_legacyPurchasePage_createdAt_idx', 'temp_OneTimePurchase_legacyPurchasePage_createdAt_idx_invalid');
  END IF;
END
$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE n.nspname = current_schema()
      ...
      AND regexp_replace(regexp_replace(pg_get_expr(i.indpred, i.indrelid), '("[^"]+"|[A-Za-z_][A-Za-z0-9_$]*)\."PurchaseCreationSource"', '"PurchaseCreationSource"', 'g'), '[()\s]', '', 'g') = regexp_replace('"creationSource" = ''PURCHASE_PAGE''::"PurchaseCreationSource" AND "paidAt" IS NULL', '[()\s]', '', 'g')
    ...
    RAISE EXCEPTION 'TV legacy purchase index did not finish with the expected valid definition';
  END IF;
END
$$;
