-- TV payment paid-at index recovery tolerates invalid remnants from interrupted concurrent builds.
...
BEGIN
  ...
  WHERE index_namespace.nspname = current_schema()
    AND index_relation.relname = 'SubscriptionInvoice_tenancyId_paidAt_idx_invalid';
  ...
  IF FOUND AND (
    ...
    OR regexp_replace(regexp_replace(existing.predicate_definition, '"[^\"]+"\."PurchaseCreationSource"', '"PurchaseCreationSource"', 'g'), '[()\s]', '', 'g') IS DISTINCT FROM regexp_replace('"paidAt" IS NOT NULL', '[()\s]', '', 'g')
    ...
    RAISE EXCEPTION 'TV payment paid-at invalid-index remnant % exists but has an unexpected definition; refusing to drop it', 'SubscriptionInvoice_tenancyId_paidAt_idx_invalid';
  END IF;
END
$$;
...
DROP INDEX CONCURRENTLY IF EXISTS /* SCHEMA_NAME_SENTINEL */."SubscriptionInvoice_tenancyId_paidAt_idx_invalid";
...
BEGIN
  ...
  LEFT JOIN pg_am access_method ON access_method.oid = index_relation.relam
  WHERE index_namespace.nspname = current_schema()
    AND index_relation.relname = 'SubscriptionInvoice_tenancyId_paidAt_idx';
  ...
  IF FOUND AND (
    ...
    RAISE EXCEPTION 'TV payment paid-at index % already exists but is invalid or has an unexpected definition; refusing to drop it', 'SubscriptionInvoice_tenancyId_paidAt_idx';
  ELSIF FOUND AND (
    existing.indisvalid IS DISTINCT FROM TRUE
    OR existing.indisready IS DISTINCT FROM TRUE
  ) THEN
    EXECUTE format('ALTER INDEX %I RENAME TO %I', 'SubscriptionInvoice_tenancyId_paidAt_idx', 'SubscriptionInvoice_tenancyId_paidAt_idx_invalid');
  END IF;
END
$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE n.nspname = current_schema()
      ...
      AND regexp_replace(regexp_replace(pg_get_expr(i.indpred, i.indrelid), '"[^\"]+"\."PurchaseCreationSource"', '"PurchaseCreationSource"', 'g'), '[()\s]', '', 'g') = regexp_replace('"paidAt" IS NOT NULL', '[()\s]', '', 'g')
    ...
    RAISE EXCEPTION 'TV payment paid-at index did not finish with the expected valid definition';
  END IF;
END
$$;
