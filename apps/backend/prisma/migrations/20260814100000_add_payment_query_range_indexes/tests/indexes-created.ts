...
export const postMigration = async (sql: Sql) => {
  ...
  expect(await sql`
    ...
    WHERE n.nspname = current_schema()
      AND idx.relname = 'SubscriptionInvoice_tenancyId_paidAt_idx'
      AND i.indisvalid
      AND i.indisready
  `).toHaveLength(1);
  ...
  try {
    ...
    await sql.unsafe(`CREATE INDEX CONCURRENTLY "SubscriptionInvoice_tenancyId_paidAt_idx" ON "SubscriptionInvoice"("createdAt")`);
    ...
    await sql.unsafe('DROP INDEX CONCURRENTLY IF EXISTS "SubscriptionInvoice_tenancyId_paidAt_idx"');
    await sql.unsafe(`CREATE INDEX CONCURRENTLY "SubscriptionInvoice_tenancyId_paidAt_idx" ON "SubscriptionInvoice"("tenancyId", "paidAt") WHERE "paidAt" IS NOT NULL`);
    await sql.unsafe(`UPDATE pg_index SET indisvalid = false, indisready = false WHERE indexrelid = (SELECT indexrelid FROM pg_class idx JOIN pg_namespace n ON n.oid = idx.relnamespace JOIN pg_index i ON i.indexrelid = idx.oid WHERE n.nspname = current_schema() AND idx.relname = 'SubscriptionInvoice_tenancyId_paidAt_idx')`);
    await executeMigration();
    expect(await sql`
      SELECT 1 FROM pg_class idx
      JOIN pg_namespace n ON n.oid = idx.relnamespace
      WHERE n.nspname = current_schema() AND idx.relname = 'SubscriptionInvoice_tenancyId_paidAt_idx_invalid'
    `).toHaveLength(0);
    expect(await sql`
    ...
    await sql.unsafe('DROP INDEX CONCURRENTLY IF EXISTS "SubscriptionInvoice_tenancyId_paidAt_idx"');
    await sql.unsafe(`CREATE INDEX CONCURRENTLY "SubscriptionInvoice_tenancyId_paidAt_idx" ON "SubscriptionInvoice"("tenancyId", "paidAt") WHERE "paidAt" IS NOT NULL`);
    await sql.unsafe(`UPDATE pg_index SET indisvalid = false, indisready = true WHERE indexrelid = (SELECT indexrelid FROM pg_class idx JOIN pg_namespace n ON n.oid = idx.relnamespace JOIN pg_index i ON i.indexrelid = idx.oid WHERE n.nspname = current_schema() AND idx.relname = 'SubscriptionInvoice_tenancyId_paidAt_idx')`);
    await executeMigration();
    expect(await sql`
      SELECT 1 FROM pg_class idx
      JOIN pg_namespace n ON n.oid = idx.relnamespace
    ...
    await sql.unsafe('DROP INDEX CONCURRENTLY IF EXISTS "SubscriptionInvoice_tenancyId_paidAt_idx_invalid"');
    await sql.unsafe('DROP INDEX CONCURRENTLY IF EXISTS "SubscriptionInvoice_tenancyId_paidAt_idx"');
    await sql.unsafe(`CREATE INDEX CONCURRENTLY "SubscriptionInvoice_tenancyId_paidAt_idx_invalid" ON "SubscriptionInvoice"("tenancyId", "paidAt") WHERE "paidAt" IS NOT NULL`);
    ...
    expect(await sql`
      ...
      JOIN pg_class idx ON idx.oid = i.indexrelid
      JOIN pg_namespace n ON n.oid = idx.relnamespace
      WHERE n.nspname = current_schema() AND idx.relname = 'SubscriptionInvoice_tenancyId_paidAt_idx_invalid' AND NOT i.indisvalid
    `).toHaveLength(1);
    ...
    await sql.unsafe('DROP INDEX CONCURRENTLY IF EXISTS "SubscriptionInvoice_tenancyId_paidAt_idx_invalid"');
    await sql.unsafe('DROP INDEX CONCURRENTLY IF EXISTS "SubscriptionInvoice_tenancyId_paidAt_idx"');
    await sql.unsafe(`CREATE INDEX CONCURRENTLY "SubscriptionInvoice_tenancyId_paidAt_idx" ON "SubscriptionInvoice"("tenancyId", "paidAt") INCLUDE ("createdAt") WHERE "paidAt" IS NOT NULL`);
    await sql.unsafe(`UPDATE pg_index SET indisvalid = false, indisready = true WHERE indexrelid = (SELECT indexrelid FROM pg_class idx JOIN pg_namespace n ON n.oid = idx.relnamespace JOIN pg_index i ON i.indexrelid = idx.oid WHERE n.nspname = current_schema() AND idx.relname = 'SubscriptionInvoice_tenancyId_paidAt_idx')`);
    await expect(executeMigration()).rejects.toThrow(/refusing to drop it/);
    expect(await sql`
      SELECT 1 FROM pg_index i
      JOIN pg_class idx ON idx.oid = i.indexrelid
      JOIN pg_namespace n ON n.oid = idx.relnamespace
      WHERE n.nspname = current_schema() AND idx.relname = 'SubscriptionInvoice_tenancyId_paidAt_idx' AND NOT i.indisvalid AND i.indisready
    `).toHaveLength(1);
  } finally {
    await sql.unsafe('DROP INDEX CONCURRENTLY IF EXISTS "SubscriptionInvoice_tenancyId_paidAt_idx_invalid"');
    await sql.unsafe('DROP INDEX CONCURRENTLY IF EXISTS "SubscriptionInvoice_tenancyId_paidAt_idx"');
