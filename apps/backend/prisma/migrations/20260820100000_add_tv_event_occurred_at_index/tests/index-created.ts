...
export const postMigration = async (sql: Sql) => {
  ...
  try {
    ...
    await sql.unsafe(`CREATE INDEX CONCURRENTLY "TvEventOccurrence_occurred_lookup_idx" ON "TvEventOccurrence"("createdAt")`);
    ...
    await sql.unsafe(`UPDATE pg_index SET indisvalid = false, indisready = false WHERE indexrelid = (SELECT indexrelid FROM pg_class idx JOIN pg_namespace n ON n.oid = idx.relnamespace JOIN pg_index i ON i.indexrelid = idx.oid WHERE n.nspname = current_schema() AND idx.relname = 'TvEventOccurrence_occurred_lookup_idx')`);
    ...
    expect(await sql`
      ...
      JOIN pg_class idx ON idx.oid = i.indexrelid
      JOIN pg_namespace n ON n.oid = idx.relnamespace
      WHERE n.nspname = current_schema() AND idx.relname = 'TvEventOccurrence_occurred_lookup_idx_invalid' AND NOT i.indisvalid
    `).toHaveLength(1);
    ...
    await sql.unsafe('DROP INDEX CONCURRENTLY IF EXISTS "TvEventOccurrence_occurred_lookup_idx_invalid"');
    await sql.unsafe('DROP INDEX CONCURRENTLY IF EXISTS "TvEventOccurrence_occurred_lookup_idx"');
    await sql.unsafe(`CREATE INDEX CONCURRENTLY "TvEventOccurrence_occurred_lookup_idx" ON "TvEventOccurrence"("tenancyId", "occurredAt" DESC, "id") INCLUDE ("createdAt")`);
    await sql.unsafe(`UPDATE pg_index SET indisvalid = false, indisready = true WHERE indexrelid = (SELECT indexrelid FROM pg_class idx JOIN pg_namespace n ON n.oid = idx.relnamespace JOIN pg_index i ON i.indexrelid = idx.oid WHERE n.nspname = current_schema() AND idx.relname = 'TvEventOccurrence_occurred_lookup_idx')`);
    await expect(executeMigration()).rejects.toThrow(/refusing to drop it/);
    expect(await sql`
      SELECT 1 FROM pg_index i
      JOIN pg_class idx ON idx.oid = i.indexrelid
      JOIN pg_namespace n ON n.oid = idx.relnamespace
      WHERE n.nspname = current_schema() AND idx.relname = 'TvEventOccurrence_occurred_lookup_idx' AND NOT i.indisvalid AND i.indisready
    `).toHaveLength(1);
  } finally {
    await sql.unsafe('DROP INDEX CONCURRENTLY IF EXISTS "TvEventOccurrence_occurred_lookup_idx_invalid"');
    await sql.unsafe('DROP INDEX CONCURRENTLY IF EXISTS "TvEventOccurrence_occurred_lookup_idx"');
