-- TV event resolved-at index recovery tolerates invalid remnants from interrupted concurrent builds.
...
BEGIN
  ...
  WHERE index_namespace.nspname = current_schema()
    AND index_relation.relname = 'TvEventOccurrence_resolved_lookup_idx_invalid';
...
DROP INDEX CONCURRENTLY IF EXISTS /* SCHEMA_NAME_SENTINEL */."TvEventOccurrence_resolved_lookup_idx_invalid";
...
BEGIN
  ...
  LEFT JOIN pg_am access_method ON access_method.oid = index_relation.relam
  WHERE index_namespace.nspname = current_schema()
    AND index_relation.relname = 'TvEventOccurrence_resolved_lookup_idx';
  ...
  IF FOUND AND (
    ...
    RAISE EXCEPTION 'TV event resolved-at index % already exists but is invalid or has an unexpected definition; refusing to drop it', 'TvEventOccurrence_resolved_lookup_idx';
  ELSIF FOUND AND (
    existing.indisvalid IS DISTINCT FROM TRUE
    OR existing.indisready IS DISTINCT FROM TRUE
  ) THEN
    EXECUTE format('ALTER INDEX %I RENAME TO %I', 'TvEventOccurrence_resolved_lookup_idx', 'TvEventOccurrence_resolved_lookup_idx_invalid');
  END IF;
END
$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'TV event resolved-at index did not finish with the expected valid definition';
  END IF;
END
$$;
