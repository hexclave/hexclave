  ...
  try {
    ...
    const [summaryRows, trendRows, sendActivity] = await Promise.all([
      ...
      metricsPrisma.$queryRaw<{ day: string, delivered: number, error: number, in_progress: number }[]>`
        ...
        ORDER BY day
      `,
      // A send-activity query failure must surface as the visible error state
      // rather than a "successful" screen with missing metrics, so it is not
      // caught separately from the summary/trend queries.
