...
describe("Fly proxy hostnames", () => {
  it("reuses the complete Fly app identity without a DNS lookup or certificate", () => {
    ...
    expect(`hxc-${hostname.slice("deploy-".length).split(".")[0]}`).toBe(appNameForService("prod", "project", "web"));
  ...
  it("reserves generated names without reserving hosted components", () => {
    expect(isPlatformHostname("Deploy-example.built-with-hexclave.com.")).toBe(true);
    expect(isPlatformHostname("project-id.built-with-hexclave.com")).toBe(false);
    expect(isPlatformHostname("deploy-test.built-with-hexclave.com.attacker.com")).toBe(false);
  });
});
